  ...
  switch (chainType(dstChain)) {
    ...
    case 'hardhat': case 'bsc': case 'eth2':
      ...
      if (bridgeMode) {
        ...
        console.log(dstChain.contracts.bmc)
        console.log(srcChain.network)
        console.log(srcChain.networkTypeId)
        console.log(firstBlockHeader)
...
async function setup_link_hardhat(src: string, srcChain: any, dstChain: any) {
  ...
  await bmcm.removeVerifier(dstChain.network)
      .then((tx) => {
        return tx.wait(1)
      });
