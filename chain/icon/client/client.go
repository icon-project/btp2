...
func (c *Client) GetBlockByHash(p *BlockHashParam) (*Block, error) {
	result := &Block{}
	if _, err := c.Do("icx_getBlockByHash", p, &result); err != nil {
		return nil, err
	}
	return result, nil
}
	...
	go func() {
		for {
			if _, ok := c.conns[conn.LocalAddr().String()]; !ok {
				...
				ech <- fmt.Errorf("wsReadJSONLoop c.conns[%s] is nil", conn.LocalAddr().String())
				break
			}
